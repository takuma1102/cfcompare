...
trajectory: the treated-unit average against the estimated `Y(0)`, with the
post-treatment gap (the effect) shaded. Because TROP carries explicit time
weights, the weights `theta_s = exp(-lambda_time * |t - s|)` are drawn as a
filled band along the bottom, in the style of `synthdid`'s time-weight strip.
The band shows *time weights* — how much each period the counterfactual leans on
— not observation counts; it is flat when `lambda_time = 0` (uniform weighting).
Use `show_weights = FALSE` to drop the band.
