...
default set --- `TROP`, `DID`, `SC`, `MC`, `SDID`, and `DIFP` (the native engines
plus synthetic DID and synthetic control). To run the defaults without a particular method, pass `exclude`
...
This trajectory shows only point estimates — no standard errors or confidence
intervals — so by default the subtitle reports just the selected penalties (set
`show_se = TRUE` to also note the SE method). To *see* the fit's uncertainty, use
`trop_event_study()` and `autoplot()` on it, which draws per-period estimates with
pointwise confidence bars (next section). The `se=` argument on `trop()` still
controls the standard error shown in `print()`, `as_att()`, and the comparison
forest plot.

```{r trop-autoplot, fig.width = 6, fig.height = 4.5, fig.alt = "TROP trajectory: observed vs. estimated Y(0) with the time-weight band along the bottom"}
autoplot(fit)
```
...
```{r trop-event, fig.width = 6, fig.height = 4.5, fig.alt = "TROP event study: per-period ATT against event time with pointwise confidence bars and pre-treatment placebo points"}
...
`trop_ablation()` runs that whole sweep of constraints for you — full TROP, then
dropping the unit weights, the time weights, the regression adjustment, and the
combinations that reduce to matrix completion and difference-in-differences — and
returns it as a table (the robustness exercise behind the paper's Table 5)
instead of a plot.
...
Print it for the console, or `format()` it to paste-ready LaTeX (booktabs) or
Markdown for a paper or report; pass `se = "jackknife"` (or any [trop()] SE
method) to add standard-error and confidence-interval columns. Because the result
is an ordinary `data.frame`, it also drops straight into `knitr::kable()`,
`gt::gt()`, and similar.

```{r ablation-latex, eval = FALSE}
...
split additively as `L_js = X_js·φ + R_js` (Section 6.2): the covariate-linear
...
estimated `Y(0)` as an `N × T` matrix, so methods can be compared on the same
grid:
...
separately, with replacement, keeping each unit's full time series), the
approach used for TROP's standard errors and confidence intervals in the paper.
Percentile intervals are the default; tune the bootstrap with
`trop_control(n_boot =, boot_ci =)`. Two alternatives are available via `se =`:
**jackknife** (leave-one-treated-unit-out; needs at least two treated units) and
**placebo** (assign the treated pattern to controls; for a single treated unit).
`se = "auto"` picks jackknife when there are at least two treated units and
placebo otherwise. These are practical approximations; see Section 6 of the
paper for the estimator's formal inference theory.
